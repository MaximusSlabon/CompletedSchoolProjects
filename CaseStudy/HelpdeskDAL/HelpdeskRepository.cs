...
{
    class HelpdeskRepository<T> : IRepository<T> where T : HelpdeskEntity
    {
        ...
        public async Task<UpdateStatus> Update(T updatedEntity)
        {
            ...
            catch (DbUpdateConcurrencyException dbx)
            {
                ...
                Console.WriteLine("Problem int " + MethodBase.GetCurrentMethod().Name + dbx.Message);
            }
            catch (Exception ex)
            {
                Console.WriteLine("Problem int " + MethodBase.GetCurrentMethod().Name + ex.Message);
            }
